...
struct AbstractField {
    ...
    virtual void init(const FieldConfig& f, const SimSetts& setts) = 0;
...
struct Field final: AbstractField {
    ...
    void init(const FieldConfig& f, const SimSetts& setts) override;
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::save(const std::string& filename, size_t i) {
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::nextTick(int i) {
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::init(const FieldConfig& f, const SimSetts& setts) {
    g = f.g; N = f.h; K = f.w;
    rho[' '] = f.rhoField;
    rho['.'] = f.rhoFluid;
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::serialize()
{
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::propagate_stop(int x, int y, bool force) {
    if (!force) {
        bool stop = true;
        for (auto [dx, dy]: deltas) {
            int nx = x + dx, ny = y + dy;
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
VType Field<PType, VType, VFType, N_val, K_val>::move_prob(int x, int y) {
    VType sum;
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::swap(int x1, int y1, int x2, int y2) {
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
std::tuple<VFType, bool, std::pair<int, int>> Field<PType, VType, VFType, N_val, K_val>::propagate_flow(int x, int y, VFType lim) {
    last_use[x][y] = UT - 1;
    VFType ret{};
    for (auto [dx, dy]: deltas) {
        int nx = x + dx, ny = y + dy;
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
bool Field<PType, VType, VFType, N_val, K_val>::propagate_move(int x, int y, bool is_first) {
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::apply_external_forces() {
    for (size_t x = 0; x < N; ++x) {
        for (size_t y = 0; y < K; ++y) {
            if (field[x][y] == '#')
                continue;
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::apply_forces_from_p(PType &total_delta_p) {
    old_p = p;
    for (size_t x = 0; x < N; ++x) {
        for (size_t y = 0; y < K; ++y) {
            if (field[x][y] == '#')
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::make_flow_from_velocities() {
    velocity_flow.clear();
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
void Field<PType, VType, VFType, N_val, K_val>::recalculate_p(PType &total_delta_p) {
    for (size_t x = 0; x < N; ++x) {
        for (size_t y = 0; y < K; ++y) {
            if (field[x][y] == '#')
                continue;
...
template<typename PType, typename VType, typename VFType, int N_val, int K_val>
bool Field<PType, VType, VFType, N_val, K_val>::apply_move_on_flow() {
    UT += 2;
    bool prop = false;
    for (size_t x = 0; x < N; ++x) {
        for (size_t y = 0; y < K; ++y) {
